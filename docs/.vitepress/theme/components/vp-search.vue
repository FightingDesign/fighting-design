...
<template>
  <div class="vp-search">
    ...
    <!-- 搜索结果 -->
    <div v-if="isShow && resultList && resultList.length" class="vp-search__result" :class="{ 'dark': isDark }"
      @click.stop="hiddenResult">
      <a v-for="(item, index) in resultList" class="vp-search__link" :class="{ 'dark': isDark }" :key="index"
        :href="`/${item.url}.html`">
        {{ item.title }}
      </a>
    </div>
